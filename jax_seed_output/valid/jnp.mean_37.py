...
def correlation(data):
    ...
    variance = (jnp.var(data, axis=0) + jnp.mean(data, axis=0))
    covariance = ((jnp.matmul(jnp.matmul(data, data.T, transpose_a=True), data.T) + jnp.mean(data, axis=0)) / np.sqrt((2 * variance)))
