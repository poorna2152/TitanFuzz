...
def head_3d(u):
    ...
    for t in range(TMAX):
        u_new = ((((((jnp.roll((jnp.roll(u, (- 2), axis=0) + jnp.roll(u, (- 1), axis=0))) + jnp.roll(u, (- 1), axis=0)) + jnp.roll(u, 1, axis=1)) + jax.roll(u, (- 1), axis=1)) + jnp.roll(u, 1, axis=2)) + jnp.roll(u, (- 1), axis=2)) / 6)
